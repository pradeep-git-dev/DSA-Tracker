...
/**
 * Maps a LeetCode submission status to the appropriate mistake type.
 * @param {string} statusDisplay - LeetCode submission status (e.g., 'Wrong Answer')
 * @returns {string}
 */
export function getMistakeTypeFromStatus(statusDisplay) {
  if (statusDisplay === "Time Limit Exceeded" || statusDisplay === "Memory Limit Exceeded") {
    return "complexity";
  }
  if (statusDisplay === "Wrong Answer") {
    return "edge-case";
  }
  return "implementation"; // fallback for Runtime Error, Compile Error, etc.
}

/**
 * Analyzes failed submissions to produce a candidate mistake object ready for insertion.
 * @param {object} submission - LeetCode submission details
 * @param {object} questionDetails - LeetCode problem details containing topic tags
 * @returns {object}
 */
export function buildMistakeCandidate(submission, questionDetails) {
  const topic = questionDetails?.topicTags?.[0]?.name || "General";
  const mistakeType = getMistakeTypeFromStatus(submission.statusDisplay);
  const intuition = getIntuition(submission.titleSlug, topic);

  // Set default initial review gap to 3 days
  ...
  return {
    ...
    root_cause: `Auto-logged from LeetCode submission: ${submission.statusDisplay} (${submission.lang})`,
    correction: `Review the problem on LeetCode and fix the ${submission.statusDisplay} error.`,
    trigger_clues: intuition.triggerClues,
    core_invariant: intuition.coreInvariant,
    common_pitfalls: intuition.commonPitfalls,
    severity: 3,
    status: "open",
