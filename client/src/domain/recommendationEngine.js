...
export function buildRecommendations({ snapshot, mistakes, patterns, topicInsights, sessions }) {
  ...
  for (const mistake of mistakes) {
    const ageDays = Math.max(1, (Date.now() - new Date(mistake.created_at || mistake.createdAt).getTime()) / 86400000);
    const reviewPressure = Math.min(30, ageDays * 1.5) + mistake.severity * 8 + Math.max(0, 4 - mistake.review_count) * 6;
  ...
  const recentSolved = new Set((snapshot?.recent_accepted || snapshot?.recentAccepted || []).map((item) => item.titleSlug));
  const failedProblems = new Map(
    (snapshot?.attempt_stats?.problemAttempts || snapshot?.attemptStats?.problemAttempts || []).map((item) => [
      item.titleSlug,
      item.failed
    ])
  );
  const failedTopics = new Map(
    (snapshot?.attempt_stats?.topicAttempts || snapshot?.attemptStats?.topicAttempts || []).map((item) => [
      item.topic,
      item.failed
    ])
  );
  const scheduledTopics = new Set(
    sessions.filter((session) => session.status === "scheduled").map((session) => session.focus_topic || session.focusTopic)
  );
...
export function generateRevisionBlueprint({ mistakes, topicInsights }) {
  ...
  const queue = Object.entries(byTopic)
    .map(([topic, topicMistakes]) => ({
      ...
      priority: topicMistakes.reduce(
        (sum, mistake) => sum + mistake.severity * 10 + Math.max(0, 4 - mistake.review_count) * 7,
        0
      )
    }))
...
function buildReport({ snapshot, mistakes, patterns, weakTopics, questions }) {
  ...
  const submissionsCount = snapshot?.counts?.submissions?.all || 0;
  const accuracy = submissionsCount ? Math.round((solved / submissionsCount) * 100) : 0;
